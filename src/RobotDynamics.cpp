...
RobotDynamics::RobotDynamics(vector<float> offset, vector<float> L_list, vector<float> D_list, vector<float> theta_list, vector<float> phi_list, int trailer_count, bool direction){
...
vector<float> RobotDynamics::calculate_mule_pose(){
    /*
    ...
    */
    ...
    vector<float> mule_position = {0.0, 0.0};

    for (int i = 0; i < trailer_count; i++) {
        float xi = this->D_list[i] * cos(this->phi_list[i]) + this->L_list[i] * cos(this->theta_list[i]);
        float yi = this->D_list[i] * sin(this->phi_list[i]) + this->L_list[i] * sin(this->theta_list[i]);
...
pair<vector<float>, vector<float>> RobotDynamics::calculate_kth_hitch_trailer_pose(int k){
    /*
    ...
    */
    ...
    float xi_trailer = this->mule_position[0];
    float yi_trailer = this->mule_position[1];
    float xi_hitch = 0.0, yi_hitch = 0.0;
    ...
    vector<float> hitch = {xi_hitch, yi_hitch};
    vector<float> trailer = {xi_trailer, yi_trailer};

    return {hitch, trailer};
}

PathPoint RobotDynamics::update_state(const vector<float> control_action, float dt){
    /*
    ...
    */
    PathPoint updated_state;

    float v = control_action[0];
    float omega = control_action[1];

    float x_mule_new = mule_position[0] + v * cos(theta_list[0]) * dt;
    float y_mule_new = mule_position[1] + v * sin(theta_list[0]) * dt;

    vector<float> new_theta_list = theta_list;
    vector<float> new_phi_list = phi_list;

    float v_ht = v;
    float omega_ht = omega;
    float v_ht_new = 0.0, omega_ht_new = 0.0;

    for (int i = 0; i < trailer_count; i++) {
        control_actions.push_back({v_ht, omega_ht});

        auto trailer_pair = calculate_kth_hitch_trailer_pose(i+1);
        vector<float> trailer_old = trailer_pair.second;

        float x_trailer_new = trailer_old[0] + v_trailer(v_ht, omega_ht, i) * cos(phi_list[i]) * dt;
        float y_trailer_new = trailer_old[1] + v_trailer(v_ht, omega_ht, i) * sin(phi_list[i]) * dt;
    ...
    updated_state.x = mule_position[0];
    updated_state.y = mule_position[1];
    updated_state.theta = mule_orientation;

    return updated_state;
}

float RobotDynamics::v_trailer(float v, float omega, int i){
    /*
...
float RobotDynamics::phi_dot(float v, float omega, int i){
    /*
...
float RobotDynamics::omega_next(float v_ht, float phi_dot, int i){
    /*
