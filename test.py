...
# ------------------------
#  Frenet utilities & planner (new, added below existing classes)
# ------------------------
def project_obstacle_to_frenet(path_func, obs_center, v=1.0, T=200.0, dt=0.01):
    """
    Sample reference path and find closest point to obstacle center.
    Returns s_obs, d_obs, t_at_min.
    """
    ox, oy = obs_center
    best_dist = float("inf")
    best_rx = best_ry = best_rtheta = 0.0
    best_t = 0.0
    samples = int(max(1, T / dt))
    for i in range(samples + 1):
        t = i * dt
        rx, ry, rtheta = path_func(t)
        dist = math.hypot(rx - ox, ry - oy)
        if dist < best_dist:
            best_dist = dist
            best_rx, best_ry, best_rtheta = rx, ry, rtheta
            best_t = t
    s_obs = v * best_t
    d_obs = math.sin(best_rtheta) * (ox - best_rx) - math.cos(best_rtheta) * (oy - best_ry)
    return s_obs, d_obs, best_t

def frenet_to_cartesian_array(s_arr, d_arr, path_func, v=1.0):
    xs = []
    ys = []
    thetas = []
    for s, d in zip(s_arr, d_arr):
        t = s / v
        rx, ry, rtheta = path_func(t)
        x = rx + d * math.sin(rtheta)
        y = ry - d * math.cos(rtheta)
        xs.append(x)
        ys.append(y)
        thetas.append(rtheta)
    return np.array(xs), np.array(ys), np.array(thetas)

def frenet_mpc_full_horizon(s0, d0, v, N, dt, path_func, obs_center, obs_radius,
                           R_inflate=1.0, weights=None, bounds_d=(-6.0, 6.0)):
    """
    Solve global Frenet optimization for N steps (decision variable: d[0:N]).
    Returns s_seq, d_opt, (s_obs, d_obs, Rf), optimizer result.
    """
    if weights is None:
        weights = {"w_d": 80.0, "w_smooth": 200.0, "w_obs": 6000.0}
    w_d = weights["w_d"]
    w_smooth = weights["w_smooth"]
    w_obs = weights["w_obs"]

    s_seq = np.array([s0 + (k + 1) * v * dt for k in range(N)])
    d_ref = np.zeros(N)

    # project obstacle
    s_obs, d_obs, _ = project_obstacle_to_frenet(path_func, obs_center, v=v, T=max(5.0, s_seq[-1]/v + 5.0), dt=dt)
    Rf = obs_radius + R_inflate
    ...
    def cost(d_seq):
        J = w_d * np.sum((d_seq - d_ref) ** 2)
        if N >= 3:
            dd = d_seq[2:] - 2.0 * d_seq[1:-1] + d_seq[:-2]
            J += w_smooth * np.sum(dd ** 2)
        d_rel = d_seq - d_obs
        dist_sq = s_diff ** 2 + d_rel ** 2
        dist = np.sqrt(dist_sq)
        violation = np.maximum(0.0, Rf - dist)
        J += w_obs * np.sum(violation ** 2)
        return J

    res = minimize(cost, x0, bounds=bnds, method='SLSQP', options={'ftol':1e-6, 'maxiter':2000})
    d_opt = res.x if res.success else x0
    return s_seq, d_opt, (s_obs, d_obs, Rf), res
...
class PathPlanningFF:
    def __init__(self, path_func, start, controller, L=1.5, D=2.0, dt=0.1, T=100):
        ...
        self.orig_path_func = path_func  # keep original reference (we may overwrite path_func later)
        ...
        # placeholder for frenet plan & obstacle info (set externally)
        self.frenet_plan = None
        self.obstacle_cart = None
    ...
    def simulate(self):
        for i in range(self.steps):
            t = i * self.dt
            # use whatever path_func is currently assigned (we may have replaced it with modified reference)
            ...
            # Important: For meaningful d relative to the original reference (d=0),
            # use orig_path_func if available; else fall back to current path_func.
            ref_pf = getattr(self, "orig_path_func", self.path_func)
            x_ref, y_ref, theta_ref = ref_pf(t)
            ...
            # store Frenet reference path (original ref)
    ...
    def animate(self):
        ...
        # If a planned path exists, draw it (converted from frenet plan)
        if self.frenet_plan is not None:
            s_plan, d_plan = self.frenet_plan
            x_plan, y_plan, _ = frenet_to_cartesian_array(s_plan, d_plan, self.orig_path_func, v=self.v)
            ax_cart.plot(x_plan, y_plan, linestyle=':', linewidth=2.0, label='Planned ref (cartesian)')

        # Plot obstacle in Cartesian (if provided)
        if self.obstacle_cart is not None:
            obs_center, obs_radius = self.obstacle_cart
            cart_circle = plt.Circle(obs_center, obs_radius, color='purple', fill=False, alpha=0.6, linewidth=1.6, label='Obstacle (cart)')
            ax_cart.add_patch(cart_circle)
            ax_cart.scatter([obs_center[0]], [obs_center[1]], marker='x', color='purple')
        ...
        ax_frenet.set_ylim(-6, 6)
        ax_frenet.axhline(0, color='gray', linestyle='--', label='Reference Path (d=0)')

        # If frenet plan exists, show its curve in s-d
        if self.frenet_plan is not None:
            s_plan, d_plan = self.frenet_plan
            ax_frenet.plot(s_plan, d_plan, linestyle=':', linewidth=2.0, label='Planned d(s)')

        # Plot obstacle in Frenet (if provided)
        if self.obstacle_frenet is not None:
            s_obs, d_obs, Rf = self.obstacle_frenet
            phi = np.linspace(0, 2 * np.pi, 200)
            sx = s_obs + Rf * np.cos(phi)
            sd = d_obs + Rf * np.sin(phi)
            ax_frenet.plot(sx, sd, color='purple', linewidth=1.6, label='Obstacle (frenet inflated)')
            ax_frenet.scatter([s_obs], [d_obs], marker='x', color='purple')
        ...
        frenet_coords = []
        s_accum = 0
        # use original ref function for frenet projection so d is relative to original path
        ref_pf = getattr(self, "orig_path_func", self.path_func)
        for i in range(len(self.states)):
            x, y = self.states[i]
            x_ref, y_ref, theta_ref = ref_pf(i * self.dt)
...
if __name__ == "__main__":
    # original parametric reference path (we keep this as orig_path_func)
    orig_path_func = lambda t: ParametricFunctions.figure_eight(t, a=10.0, v=1.0)

    # instantiate controller (unchanged)
    controller = Controller(N=10, dt=0.1, v=1.0)

    # create PathPlanningFF with the original reference for now
    sim = PathPlanningFF(orig_path_func, (0, 0, np.pi/4), controller, L=1.5, D=2.0, dt=0.1, T=100.0)

    # ===== compute a global frenet plan (one-shot) for the full sim horizon =====
    # obstacle in Cartesian
    obs_center = (10.0, 0.0)
    obs_radius = 1.5
    R_inflate = 1.0  # inflate to account for mule+hitch+trailer footprint

    # call the frenet planner for N = sim.steps
    s0 = 0.0
    d0 = 0.0
    N_plan = sim.steps
    s_seq, d_opt, (s_obs, d_obs, Rf), res_plan = frenet_mpc_full_horizon(
        s0=s0, d0=d0, v=sim.v, N=N_plan, dt=sim.dt,
        path_func=sim.orig_path_func, obs_center=obs_center, obs_radius=obs_radius,
        R_inflate=R_inflate,
        weights={"w_d": 80.0, "w_smooth": 200.0, "w_obs": 6000.0},
        bounds_d=(-6.0, 6.0)
    )

    # store plan and obstacle info in sim for plotting
    sim.frenet_plan = (s_seq, d_opt)
    sim.obstacle_cart = (obs_center, obs_radius)
    sim.obstacle_frenet = (s_obs, d_obs, Rf)

    # build a modified path_func that maps time t -> (x_mod, y_mod, theta_ref)
    def path_func_modified(t):
        # base reference at same s = v * t
        rx, ry, rtheta = sim.orig_path_func(t)
        s = sim.v * t
        # interpolate d from the global plan
        d_interp = float(np.interp(s, s_seq, d_opt, left=d_opt[0], right=d_opt[-1]))
        x_mod = rx + d_interp * math.sin(rtheta)
        y_mod = ry - d_interp * math.cos(rtheta)
        # keep reference heading (approx)
        return x_mod, y_mod, rtheta

    # assign modified path as the active reference that the Controller will track
    sim.path_func = path_func_modified
    # keep orig_path_func available for frenet logging/plotting (already stored in sim.orig_path_func)

    # simulate and animate using your existing structure
    sim.simulate()
    sim.animate()
